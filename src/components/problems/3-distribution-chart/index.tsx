...
import useHistoryData, { ScoreWithUser, UserData } from "./useDistrubutionData";
  ...
  const scales: Scales = useMemo(() => {
    return {
      ...
      y: d3
        ...
        .domain([
          0,
          d3.max(data, (user) => d3.max(user.scores, (d) => d.value)) ?? 0,
        ])
        .range([height, 0]),
    };
  }, [data, width, height]);
...
const ScatterPlots = ({
  ...
  useEffect(() => {
    ...
    dataContainer
      .selectAll("circle")
      .data(
        data.flatMap((userData) => userData.scores),
...
const StatsPlots = ({ scales, data }: { scales: Scales; data: UserData[] }) => {
  ...
  useEffect(() => {
    ...
    const stats = data.map((userData) => ({
      ...userData,
      mean: d3.mean(userData.scores.map((d) => d.value)) ?? 0,
      median: d3.median(userData.scores.map((d) => d.value)) ?? 0,
      firstQuantile:
        d3.quantile(
          userData.scores.map((d) => d.value),
          0.25
        ) ?? 0,
      thirdQuantile:
        d3.quantile(
