...
import useTop10Data, { BarData } from "./useTop10Data";
import { interpolateRdYlGn, schemeCategory10 } from "d3-scale-chromatic";
...
const YAxis = ({ yScale, data }: { yScale: Scales["y"]; data: BarData[] }) => {
  ...
  useEffect(() => {
    const yAxis = d3
      ...
      .tickSize(0)
      .tickFormat(
        (_, index) => data[index]?.data.user.name?.split(" ")[0] ?? ""
      );
...
const Bars = ({ scales, data }: { scales: Scales; data: BarData[] }) => {
      ...
      .join(
        (enter) =>
          enter
            ...
            .attr("x", 10)
            .attr("y", (d) => scales.y(d.key)! + scales.y.bandwidth() / 2)
            .text((d) => d.value)
            .call((_enter) =>
              _enter
